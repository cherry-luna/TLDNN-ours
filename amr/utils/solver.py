...
class Trainer:
    ...
    def _iteration(self, data_loader):
        ...
        for batch_idx, batch in enumerate(data_loader):
            X = batch['signal']
            Y = batch['modulation_type']
            Y = (Y - 1).clone().detach().to(dtype=torch.long) #! 为了适应mode_type的范围
        ...
        if (self.loss_name == "loss_CE_and_MSE") | (self.loss_name == "MixContrastiveLoss"):
            if self.model.training:
                logger.info(f'Train | '
                            ...
                            f'loss_CE: {iter_loss_CE.avg:.4e} | '
                            f'loss_MSE: {iter_loss_MSE.avg:.4e} | '
